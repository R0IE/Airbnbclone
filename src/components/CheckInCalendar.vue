        ...
        <div class="calendar-months">
          <div class="calendar-month">
            ...
            <div class="calendar-grid">
              <div v-for="d in daysSept" :key="d" :class="['calendar-day', d===selected ? 'selected' : '']" @click="selectDay(d)">
                <span v-if="selected === d" class="dot"></span>
                {{ d }}
              </div>
            </div>
          </div>
...
const props = defineProps({ open: Boolean, selected: Number });
const emit = defineEmits(['select']);
const tab = ref('dates');
const selectedDay = ref(props.selected ?? 17);
